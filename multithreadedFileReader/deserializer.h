...
namespace impl
{
	...
	bool is_equal(Number first, Number second)
	{
		return first == second;
	}

	template<>
	inline bool is_equal(double first, double second)
	{
		return std::abs(first - second) < std::numeric_limits<double>::epsilon();
	}

	template<typename Number>
	Number do_operation(Number first, Number second, const impl::command input_command)
	{
